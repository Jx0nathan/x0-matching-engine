...
> **当前状态：M4.2（衍生品 - 平仓 + 已实现盈亏）完成。** 衍生品订单可减仓/全平，盈亏在成交价上实时实现并加/减用户余额。资金守恒采用 mark-to-market 模型（`total_internal_with_marks`）——任意固定 mark 下，跨所有成交保持不变。M4.3 加翻仓/强平/资金费率。路线图详见 `CLAUDE.md`。
